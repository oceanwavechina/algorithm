...
typedef vector< pair<uint32_t/*start*/, uint32_t/*end*/> > RangeVec;


RangeVec find_all_subarray_with_0_sum(vector<int32_t> nums) 
{
    RangeVec all_sub_arrays;

    // 存放所有从index=0 开始的子数组的和
    unordered_map<int32_t/*sum*/, vector<uint32_t>/*endpos*/> sum_with_endpos_map;
    ...
    for(int i=0; i<nums.size(); ++i) {
        ...
        if(pre_same_sums != sum_with_endpos_map.end()) {
            ...
            for(auto pre_pos : pre_same_sums->second) {
                all_sub_arrays.push_back({pre_pos+1, i});
            }
            cout << endl << endl;
        }
